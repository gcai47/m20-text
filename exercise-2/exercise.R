...
course.descriptions <- page %>% html_nodes("p") %>% html_text()
classes <- data.frame(title = course.titles, descriptions = course.descriptions[2:length(course.descriptions)], stringsAsFactors = FALSE)

# How many courses are in the catalogue?
num.courses <- nrow(classes)

# Create a tidytext sturcture of all words
all.words <- classes %>% unnest_tokens(word, descriptions)
